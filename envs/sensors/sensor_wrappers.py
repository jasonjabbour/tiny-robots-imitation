""""Wrapper classes for extending sensor information to include history"""
...
class SensorWrapper(sensor.BoxSpaceSensor):
    ...
    def on_terminate(self, env) -> None:
        ...
        Args:
            env: the environment who envokes this callback function.
        """
...
class HistoricSensorWrapper(SensorWrapper):
    ...
    def __init__(self, 
        ...
        other world, the most recent observation is the first item in the history buffer.

        Args:
            wrapped_sensor: an inner sensor that you wrap around
            num_history: the history of sensors want to maintain
            append_history_axis: if True, add an extra axis at the end of the
                observation array for hisotry. If False, stack the historical
        ...
        # Initialize the HistoricSensorWrapper which is a subclass to the BoxSpaceSensor which is a subclass to the SensorClass
    ...
    def on_reset(self, env) -> None:
        ...
        # At reset create a new history buffer
        self._history_buffer = collections.deque(maxlen=self._num_history)

        # At reset get the same observatioin _num_history amount of times (zeros typically)
    ...
    def on_step(self, env):
        ...
        #Append new observation to the left
        self._history_buffer.appendleft(self._wrapped_sensor.get_observation())
